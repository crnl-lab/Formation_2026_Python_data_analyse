

**16 Mars 2026 - 9h00>13h00**

Sam + Alexandra + Anne + Alexandre

   * **intro python/anaconda/uv** 20min Sam
   * **jupyter/jupyterlab vs vscode vs positron** 20min Sam
   * **Base_language** 70min Sam
   * **numpy** 40min Sam
   * **matplotlib** 60min Alexandre / Anne
   * **plotly** 20min Anne


**17 Mars 2026 - 9h00>13h00**

Sam + Alexandra + Anne

  * **pandas_base** 45min Anne
  * **pandas_exo** 80min Anne
  * **seaborn_base + pingouin ???** 20min
  * **polars** 10min Sam
  * **file_manipulation** 60min Alexandra




**18 Mars 2026 - 9h00>13h00**

Sam + Alexandra + Anne

  * **Exo avec EEG** 45min Alexandra
  * **MNE overview** 45min Alexandra
  * **scipy** 30min Anne
  * **neo** 10min Sam
  * **ephyviewer** 10min Sam
  * **physio** 10min Sam



**19 Mars 2026 - 9h00>13h00**

Sam + Alexandra + Anne + Alexandre

   * **sklearn** 60min Sam / Alexandre
   * **skimage** 10min Sam
   * **networkx** 15min Anne
   * **Formula_statsmodels** 30min Alexandre
   * **xarray** 20min Sam
   * **torch** 20min Sam
   * **panel ou dash** 30min Alexandre
   * **numba ???** <<<< Sam


**20 Mars 2026 - 9h00>13h00**

  * Cluster : ssh, jupyter, venv, uv, slurm

